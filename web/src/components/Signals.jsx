          ...
          return (
            <div key={i} className={`border-l-2 ${st.border} bg-zinc-900/40 rounded-r-md pl-3 pr-3 py-2`}>
              <div className="flex items-start justify-between gap-3">
                ...
                <span className={`shrink-0 text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded ${st.chip}`}>
                  {st.label}
                </span>
              </div>
              {s.source_url && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1 mt-1 text-xs text-zinc-500 hover:text-amber-400"
                >
